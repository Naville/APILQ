...
OSStatus GenerateThumbnailForURL(void *thisInterface, QLThumbnailRequestRef thumbnail, CFURLRef url, CFStringRef contentTypeUTI, CFDictionaryRef options, CGSize maxSize)
{
    ...
    OZZipFile* ipaZip=[[OZZipFile alloc] initWithFileName:[(__bridge NSURL*)url path] mode:OZZipFileModeUnzip];
    ...
    for (OZFileInZipInfo *info in infos) {
        if([info.name hasPrefix:@"Payload/"] &&[info.name hasSuffix:@".app/"]){
            BundleRoot=info.name;
            break;
        }
    }
    if(BundleRoot!=nil){
        NSString* InfoPlistPath=[BundleRoot stringByAppendingString:@"Info.plist"];
        if([ipaZip locateFileInZip:InfoPlistPath]==YES){
            ...
            NSError * err=nil;
            NSDictionary* Info=(NSDictionary*)[NSPropertyListSerialization propertyListWithData:data options:NSPropertyListImmutable format:nil error:&err];
            ...
            if([ipaZip locateFileInZip:iconName]==YES){
                ...
                [read readDataWithBuffer:data];
                [read finishedReading];
                CGImageSourceRef ImageSrcRef=CGImageSourceCreateWithData(CFBridgingRetain(data),NULL);
                CFStringRef keys[]={kCGImageSourceCreateThumbnailFromImageAlways,kCGImageSourceCreateThumbnailWithTransform,kCGImageSourceShouldCache};
                CFTypeRef values[]={kCFBooleanTrue,kCFBooleanTrue,kCFBooleanTrue};
                CFDictionaryRef options= CFDictionaryCreate(kCFAllocatorDefault, (const void **)keys, values, 3, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
                CGImageRef ImageRef=CGImageSourceCreateThumbnailAtIndex(ImageSrcRef, 0,options);
